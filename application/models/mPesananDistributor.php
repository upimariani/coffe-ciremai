...
class mPesananDistributor extends CI_Model
{
    ...
    {
        $data['pesanan'] = $this->db->query("SELECT * FROM transaksi_distributor JOIN detail_tdistributor ON transaksi_distributor.id_tdistributor = detail_tdistributor.id_tdistibutor JOIN bahan_jadi ON bahan_jadi.id_bahan_jadi = detail_tdistributor.id_bahan_jadi WHERE transaksi_distributor.id_tdistributor='" . $id . "'")->result();
